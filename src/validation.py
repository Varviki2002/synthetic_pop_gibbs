...
class Validation:
    def __init__(self, creat_class: CondCreat, names):
        ...
        self.cross_gibbs = self.creat_class.create_cross_tables(self.gibbs_data, self.names, 0, None, None)
    ...
    def plot_lin_regression(self, x, y, xlabel):
        ...
        plt.legend([f"R^2:{model.rsquared.round(5)}", f"y = {model.params[1].round(5)}x"])
        plt.text(0, 200000, f"R^2:{model.rsquared.round(5)}", bbox=dict(facecolor='red', alpha=0.2))
...
def main():
    ...
    validation = Validation(creat_class=CondCreat(table=data, evidence=evidence, full_names=names), names=names)
