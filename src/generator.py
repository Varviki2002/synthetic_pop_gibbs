...
class SyntheticPopulationGenerator:
    ...
    def run_gibbs_sampling_census(self):
        ...
        cond_1 = pd.read_csv(
            os.path.join(data_generated, "full_cross_table_1.csv"),
            header=[0, 1, 2, 3, 4],
            index_col=[0])
        cond_2 = pd.read_csv(
            os.path.join(data_generated, "full_cross_table_2.csv"),
            header=[0, 1, 2, 3, 4],
            index_col=[0])
        cond_3 = pd.read_csv(
            os.path.join(data_generated, "full_cross_table_3.csv"),
            header=[0, 1, 2, 3, 4],
            index_col=[0])
        cond_4 = pd.read_csv(
            os.path.join(data_generated, "full_cross_table_4.csv"),
            header=[0, 1, 2, 3, 4],
            index_col=[0])
        cond_5 = pd.read_csv(
            os.path.join(data_generated, "full_cross_table_5.csv"),
            header=[0, 1, 2, 3, 4],
            index_col=[0])
        cond_6 = pd.read_csv(
            os.path.join(data_generated, "full_cross_table_6.csv"),
            header=[0, 1, 2, 3, 4],
            index_col=[0])
        partial = pd.read_csv(
            os.path.join(data_generated, "partial_cross_1.csv"),
            header=[0, 1, 2, 3],
            index_col=[0])
        ...
        student = BayesianNetwork(
            [("agegrp", "Sex"),
             ...
             ("TotInc", "hhsize")])
        cpd_age = TabularCPD(
            variable="agegrp", variable_card=18,
            values=cond_1.values,
            evidence=self.full_evidence["agegrp"],
            evidence_card=[2, 3, 3, 4, 5])
        cpd_sex = TabularCPD(
            variable="Sex", variable_card=2,
            values=cond_2.values,
            evidence=self.full_evidence["Sex"],
            evidence_card=[18, 3, 3, 4, 5])
        cpd_hdgree = TabularCPD(
            variable="hdgree", variable_card=3,
            values=cond_3.values,
            evidence=self.full_evidence["hdgree"],
            evidence_card=[18, 2, 3, 4, 5])
        cpd_lfact = TabularCPD(
            variable="lfact", variable_card=3,
            values=cond_4.values,
            evidence=self.full_evidence["lfact"],
            evidence_card=[18, 2, 3, 4, 5])
        cpd_totinc = TabularCPD(
            variable="TotInc", variable_card=4,
            values=cond_5.values,
            evidence=self.full_evidence["TotInc"],
            evidence_card=[18, 2, 3, 3, 5])
        cpd_hhsize = TabularCPD(
            variable="hhsize", variable_card=5,
            values=cond_6.values,
            evidence=self.full_evidence["hhsize"],
            evidence_card=[18, 2, 3, 3, 4])
        student.add_cpds(
            cpd_age, cpd_sex, cpd_lfact, cpd_totinc, cpd_hdgree, cpd_hhsize)
        ...
        cpd_partial_1 = TabularCPD(
            variable="agegrp", variable_card=18, values=partial.values,
            evidence=self.partial_evidence["agegrp"], evidence_card=[3, 3, 4, 5])
        population_partial.add_cpds(
            cpd_partial_1, cpd_sex, cpd_lfact, cpd_totinc, cpd_hdgree, cpd_age)
        ...
        samples_partial.to_csv(
            os.path.join(data_generated, "samples_partial.csv"), index=False)
